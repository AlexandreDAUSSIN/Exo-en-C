...
int main(void) {
  ...
  //
  holdall *count = holdall_empty();
  int z = 0;
  if (ht == NULL || has == NULL || hacptr == NULL || count == NULL) {
    goto error_capacity;
  }
  while (z != B) {
    holdall_put(count, z);
  ...
  while (scanf("%" XSTR(WORD_LENGTH_MAX) "s", w) == 1) {
    ...
    long int *cptr = (long int *) hashtable_search(ht, w);
    if (cptr != NULL) {
      *cptr += 1;
    } else {
      char *s = count(strlen(w) + 1);
      if (s == NULL) {
        goto error_capacity;
      }
      strcpy(s, w);
